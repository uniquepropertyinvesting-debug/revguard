    ...
    <div style={{ background: 'var(--bg-primary)', minHeight: '100vh', overflowX: 'hidden' }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
          ...
          <a href="/how-it-works"
            ...
            onMouseLeave={e => (e.target as HTMLElement).style.color = 'var(--text-secondary)'}
          >How It Works</a>
          <button className="btn-secondary" onClick={handleSignIn} disabled={loading} style={{ padding: '8px 16px', fontSize: '13px' }}>
            Sign In
          </button>
          <button className="btn-primary" onClick={handleGetStarted} disabled={loading} style={{ padding: '8px 20px' }}>
      ...
      <section style={{ textAlign: 'center', padding: '80px 24px 60px', maxWidth: '900px', margin: '0 auto' }}>
        ...
        {authMode === 'landing' ? (
          ...
          <div style={{ maxWidth: '380px', margin: '0 auto', marginBottom: '16px' }}>
            {authError && (
            ...
            <button
              ...
              disabled={loading}
              style={{ width: '100%', fontSize: '15px', padding: '12px', borderRadius: '10px', marginBottom: '8px' }}
            >
              {loading ? 'Please wait...' : authMode === 'signin' ? 'Sign In' : 'Create Account'}
            </button>
            <div style={{ fontSize: '13px', color: 'var(--text-muted)', textAlign: 'center' }}>
              {authMode === 'signin' ? (
                <>Don&apos;t have an account? <button onClick={() => setAuthMode('signup')} style={{ color: '#3b82f6', background: 'none', border: 'none', cursor: 'pointer', fontSize: '13px' }}>Sign up</button></>
              ) : (
                <>Already have an account? <button onClick={() => setAuthMode('signin')} style={{ color: '#3b82f6', background: 'none', border: 'none', cursor: 'pointer', fontSize: '13px' }}>Sign in</button></>
              )}
            </div>
          </div>
        )}
